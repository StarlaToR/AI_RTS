...
public class UnitController : MonoBehaviour
{
    ...
    public ETeam GetAdversaryTeam() { return Team == ETeam.Blue ? ETeam.Red : ETeam.Blue; }
    ...
    public void CaptureTarget(int points,bool isCounting = true)
    {
        Debug.Log("CaptureTarget");
        TotalBuildPoints += points;
        if(isCounting)CapturedTargets++;
    }
    ...
    protected bool RequestFactoryBuild(int factoryIndex, Vector3 buildPos, bool isIA = false)
    {
        if (SelectedFactory == null && !isIA)
            return false;
