...
public class Unit : BaseEntity
{
    ...
    public enum UnitSquadState
    {
        RESERVE,
        FREE,
        OCCUPIED,
    }
    public UnitSquadState state;
    ...
    public void SetTargetPos(Vector3 pos)
    {
        ...
        if (NavMeshAgent)
        {
             bool state = NavMeshAgent.SetDestination(pos);
            Vector3 testPost = transform.position;
            if(NavMeshAgent.pathEndPosition == testPost)
            {
                Debug.Log("Finish");
            }
