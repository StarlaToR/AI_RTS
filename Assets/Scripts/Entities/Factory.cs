...
public sealed class Factory : BaseEntity
{
    ...
    public int buildPointGain = 2;
    public float timeBetweenBuildPoint = 5;
    private float m_timerBetweenBuildPoint = 0.0f;
    public FactoryPriority factoryPriority = FactoryPriority.SECONDARY;
    ...
    public Action<Factory> OnDestroFactoryEvent;
    ...
    override protected void Update()
    {
        ...
        if (m_timerBetweenBuildPoint > timeBetweenBuildPoint)
        {
            UnitController teamController = GameServices.GetControllerByTeam(Team);
            if (teamController != null)
                teamController.CaptureTarget(buildPointGain,false) ;

            m_timerBetweenBuildPoint = 0;
        }
        else
        {
            m_timerBetweenBuildPoint += Time.deltaTime;
        }
    }
    #endregion
    void Factory_OnDead()
    {
        OnDestroFactoryEvent?.Invoke(this);
    ...
    public int GetQueueSize() { return BuildingQueue.Count; }
