...
public abstract class BaseEntity : MonoBehaviour, ISelectable, IDamageable, IRepairable
{
    ...
    public Action<Vector3> IsUnderAtack;
    ...
    public Action<BaseEntity> OnDeadEvent2;
    ...
    public bool IsLowHP() { return HP < 10; }
    ...
    public void AddDamage(int damageAmount)
    {
        ...
        OnHpUpdated?.Invoke();
        IsUnderAtack?.Invoke(transform.position);

        if (HP <= 0)
        {
            IsAlive = false;
            OnDeadEvent?.Invoke();
            OnDeadEvent2?.Invoke(this);
